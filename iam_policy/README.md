# Terraform AWS IAM Policies Infrastructure

Infrastructure-as-code for the AWS IAM Policies.

## Table of contents
- [Terraform AWS IAM Policies Infrastructure](#terraform-aws-iam-policies-infrastructure)
  - [Table of contents](#table-of-contents)
  - [Overview](#overview)
  - [Technologies](#technologies)
  - [Configuration Files](#configuration-files)
    - [config.s3.tfbackend](#configs3tfbackend)
    - [policies.tfvars.json](#policiestfvarsjson)
...
## Overview
This folder contains all the configuration files required to create AWS IAM Policies. This README will walk you through the configuration and initialization steps in order to use and maintain the respective Terraform HCL.
...
`policies.tfvars.json` defines the configuration of AWS IAM Policies using a Map(Object) expressed as JSON. The contents enclosed in curly brackets for `policy1` are a single object, if you wish to create more simply add a trailing comma after the closing curly bracket for `policy1` copy the text and paste it below giving it the title `policy(x)` based on the level of iteration and modify the arguments to support your new object. (Look to local copy for a working example)

For resources and examples on `aws_iam_policy` they can be found at the following link <a href="https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_policy">Terraform Resource - AWS IAM Policy</a>

    {
        "policies":{
            "policy1":{
                ...
                "actions": [
                        <permissions_the_policy_provides_access_to>
                ],
                "resources": [
                        <resources_this_policy_is_limited_to>
                ]
            }
        }
    }
...
After making desired changes to the Terraform files, verify that Terraform will be able to compile them correctly for deployment by running the following command:
    ...
    terraform plan -var-file="./variables/variables.tfvars" -var-file="./variables/policies.tfvars.json" -out="tfplan"
...
After migrating the state back to your local system you can destroy the AWS IAM Policies found in `policiess.tfvars.json` by running the following command:

    terraform destroy -var-file="./variables/variables.tfvars" -var-file="./variables/policies.tfvars.json" 
...
[Main |](../README.md)[ S3 |](../s3/README.md)[ IRSA |](../irsa/README.md)[ IAM Role |](../iam_role/README.md)[ IAM Group |](../iam_group/README.md)[ IAM User |](../iam_user/README.md)[ IAM Original](../iam_original/README.md)
